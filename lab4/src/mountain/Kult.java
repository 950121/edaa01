...
public class Kult extends Fractal {
	private int length;
	...
	private void fractalTriangle(TurtleGraphics turtle, int order, Point p, Point q, Point r, double dev) {
		...
		if (order == 0) {
			...
			fractalTriangle(turtle, order-1, p, q, a, dev/2);
			//andra delen "mitten"
			fractalTriangle(turtle, order-1, q, r, b, dev/2);
			//tredje delen "toppen"
			fractalTriangle(turtle, order-1, r, p, c, dev/2);
			//fjärde delen "höger"
